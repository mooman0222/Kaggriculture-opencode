...
_FIN=lambda r: 14 if (r or 0)>=13 else 2
def _router(observation,step,state):
    if step>=144 and not state.get('day6'):
        shops=_get(_get(observation,'town',{}),'unlocked_shops',[]) or []
        state['route']={('BAKERY', 'BAKERY'): 15, ('BAKERY', 'BRUNCH_SPOT'): 16, ('BAKERY', 'YARN_STORE'): 3, ('BRUNCH_SPOT', 'BRUNCH_SPOT'): 16, ('BRUNCH_SPOT', 'PET_CAFE'): 15, ('BRUNCH_SPOT', 'YARN_STORE'): 22, ('FARMERS_MARKET', 'BAKERY'): 24, ('FARMERS_MARKET', 'YARN_STORE'): 5, ('ICE_CREAM_SHOP', 'YARN_STORE'): 6, ('PET_CAFE', 'BAKERY'): 16, ('PET_CAFE', 'BRUNCH_SPOT'): 29, ('PET_CAFE', 'PET_CAFE'): 31, ('PET_CAFE', 'YARN_STORE'): 5, ('PIZZA_SHOP', 'YARN_STORE'): 7, ('SMOOTHIE_SHOP', 'YARN_STORE'): 8, ('YARN_STORE', 'BAKERY'): 9, ('YARN_STORE', 'BRUNCH_SPOT'): 9, ('YARN_STORE', 'FARMERS_MARKET'): 40, ('YARN_STORE', 'ICE_CREAM_SHOP'): 9, ('YARN_STORE', 'PET_CAFE'): 10, ('YARN_STORE', 'PIZZA_SHOP'): 6, ('YARN_STORE', 'SMOOTHIE_SHOP'): 11, ('YARN_STORE', 'YARN_STORE'): 12}.get(tuple(shops[:2]),0)
        state['day6']=True
    if step>=648 and not state.get('day27'):
        state['route']=_FIN(state.get('route',0))
        state['day27']=True
...
def _shadow_terminal(obs,config):
    ...
    if not state or state.get('last_step')!=711 or state.get('route') not in (2,14):
        return None
...
def _r51_input_forecast(obs,route,expected):
    ...
    for t in range(step,min(712,(day+4)*24)):
        tape=_IMPL.chassis.routes[_FIN(route) if t>=648 else route];a=tape[t]
...
def _r51_close_warehouse(obs,action):
    ...
    if needed>0:
        ...
        for t in range(step+1,719):
            future=_IMPL.chassis.routes[_FIN(native['route']) if t>=648 else native['route']][t]
...
def _r85_reserve(obs, state):
    ...
    if route not in cache:
        ...
        for t in range(718,-1,-1):
            a=_IMPL.chassis.routes[_FIN(route) if t>=648 else route][t]
...
def _r86_next_feed(obs, target):
    ...
    player=int(obs['player']);native=_IMPL.chassis.players[player]
    tomorrow=day+1;route=_FIN(native['route']) if tomorrow>=27 else native['route'];key=(route,tomorrow)
...
def _r97_supply(obs,action):
    ...
    native=_IMPL.chassis.players[player]
    future=_IMPL.chassis.routes[_FIN(native['route']) if step+1>=648 else native['route']][step+1]
    commands=[future.get('farmer') or ['PASS'],*(future.get('hands') or [])]
    following=_IMPL.chassis.routes[_FIN(native['route']) if step+2>=648 else native['route']][step+2]
...
def _r128_future(obs,offset=1):
    ...
    return _IMPL.chassis.routes[_FIN(native['route']) if step>=648 else native['route']][step]
