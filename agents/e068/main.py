"""E068: E066 (ahmedberatozer v46 シャシー + MMN0222 ライブ層) に夜明けガードを追加。 日跨ぎでユニットの手持ちは shed に移るが容量 100 を超えた分は破棄される (sim.hpp drop_inventories)。 実戦 30 局で 6.5 個/局・BASE 換算 約 $527 が消えていた。h22 以降、破棄される見込みの分だけ棚から高値順に先売りして場所を空ける (市場側のみ・追加のみ、小麦は給餌原料なので最後)。対 素の v46 のペア差 +248±24 (t=10.3、tune seed) / +244±29 (t=8.3、held-out)。"""
...
class Live:
    def __init__(self, folder):
        self.b = _load(folder / "base.py", "e068_base")
        self.reset()
