"""E078: E072 + 夜明けガードの過小カバー修正 (h23生産分を見込んでマージン上乗せ)。jojo局で10個破棄の実測に対応。"""
...
DAWN_MARGIN = int(os.environ.get("LA_DAWN_MARGIN", "6"))  # h23 の生産で売り後に積み増す分を見込んだ上乗せ (jojo 局で 6 個不足の実測)
SHED_CAP = 100
...
class Live:
    def __init__(self, folder):
        self.b = _load(folder / "base.py", "e078_base")
    ...
    def _dawn_guard(self, act, obs, step):
        ...
        if overflow <= 0:
            return
        overflow += DAWN_MARGIN  # h23 の生産で売り後に積み増す分 (発火時のみ上乗せ。常時売りはしない)
