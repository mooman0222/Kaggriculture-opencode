"""E067: E066 (v46 シャシー + E065 ライブ層) のルーターを差し替える実験台。 実戦 120 戦で YARN 世界 72% 勝率 / 非 YARN 世界 51% と符号が反転しており、 非 YARN では新 EXP240 表 (64 世界中 21 が route 105)、YARN では旧 V39 表 (49 が route 0) が使われている。 RT 環境変数: stock (既定=E066) / old (常に旧 V39 表) / r0 (常に route 0)。"""
...
RT = os.environ.get("RT", "stock")
DAWN = int(os.environ.get("DAWN", "0"))   # 夜明けガード: 0=off、それ以外は発動する時刻 (例 22)
SHED_CAP = 100


def _patch_router(b):
    """非 YARN 世界のルート選択だけを差し替える (YARN 世界と day27 の固定は元のまま)。"""
    if RT == "stock":
        return
    original = b._IMPL.chassis.router

    def router(observation, step, state):
        route = original(observation, step, state)
        if state.get("expert") == "EXP240":
            shops = tuple((((observation.get("town") or {}).get("unlocked_shops")) or [])[:2])
            route = 0 if RT == "r0" else b._R110_OLD_SHOPS.get(shops, 0)
            if state.get("day27"):
                route = 2
            state["route"] = route
        return route

    b._IMPL.chassis.router = router


class Live:
    def __init__(self, folder):
        self.b = _load(folder / "base.py", "e067_base")
        _patch_router(self.b)
        self.reset()
    ...
    def _dawn_guard(self, act, obs, step):
        ...
        view = self._view(obs, seat)
        shed = self._stock(act, view)                      # この手のユニット行動を織り込んだ棚
        held = {}
        ...
        if overflow <= 0:
            return
        prices = view.prices
        # 価格/BASE の高い順に棚から売って場所を空ける。小麦は給餌原料なので最後。
