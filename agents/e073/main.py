"""E073: E071 と同一 (v49 + 夜明けガード) + 709f 開幕移植 (LA_OPEN709F=1 で有効。既定 off)。
移植内容 (obs-step 基準。リプレイの t1/t2/t3 に対応):
 step0 [BUY W7, SELL W2] -> [BUY W20, SELL W15] (BUY先行の順序を維持。SELL_FIRSTソートを迂回)
 step1 [BUY W30, HIRE×5, COW2, SHEEP2] -> 小麦BUYを除去 (HIRE×5は相手と同数のため維持)
 step2 [SELL W30] -> 小麦SELLを除去 (買いなしに対応。失敗SELLは無害だが忠実化のため)
注意: UMG開幕移植 -87k の前例あり。A/B (kag_eval 32戦ペア差 + 実戦席差し替え) で悪化なら棄却。"""
...
OPEN709F = os.environ.get("LA_OPEN709F", "0") == "1"  # 709f開幕移植 (E073実験用。既定off)
...
class Live:
    def __init__(self, folder):
        self.b = _load(folder / "base.py", "e073_base")
        self.reset()
    ...
    def _open709f(self, act, step):
        """709f開幕移植。置換リストを返し、対象外stepは None。
        step0: [BUY W7, SELL W2] -> [BUY W20, SELL W15] (BUY先行を維持)
        step1: 小麦BUYを除去 (HIRE/動物は維持。相手と同数)
        step2: 小麦SELLを除去 (対応する買いがないため)"""
        if step == 0:
            return [["BUY_PRODUCT", "WHEAT", 20], ["SELL", "WHEAT", 15]]
        if step == 1:
            return [o for o in (act.get("market") or [])
                    if not (o and o[0] == "BUY_PRODUCT" and len(o) > 1 and o[1] == "WHEAT")]
        if step == 2:
            return [o for o in (act.get("market") or [])
                    if not (o and o[0] == "SELL" and len(o) > 1 and o[1] == "WHEAT")]
        return None
