"""E080: E072 (v50+層) + buy-mirror 層 (h22/h23 の BUY 流入相殺 wash)。"""
...
MIRROR = os.environ.get("LA_BUYMIRROR", "1") == "1"  # h22/h23 の BUY 流入相殺 wash (E079)
...
class Live:
    def __init__(self, folder):
        self.b = _load(folder / "base.py", "e080_base")
        # v50 は live が末尾定義 (_e343_agent) で b.agent は旧エントリの別名。最終 callable を live とする (v49/v46 では agent と同一)。
        _live = self.b.agent
        for _v in vars(self.b).values():
            if callable(_v):
                _live = _v
        self.live = _live
        self.reset()
    ...
    def act(self, obs):
        ...
        try:
            ...
            if MIRROR: self._buy_mirror(act, obs, step)
        except Exception:
            pass
        try:
            self.prev_sells = {}
    ...
    def _buy_mirror(self, act, obs, step):
        # h22/h23 のみ: テープの BUY_PRODUCT と同量の SELL を同 step に追加する
        # wash (net≈0) で、空棚の夜の買い流入 overflow を塞ぐ。テープの SELL・順序
        # は不変 (append のみで 10 件 cap の切られ側)。小麦は飼料 reserve を残す。
        if step % 24 not in (22, 23) or step >= LAST_STEP:
            return
        ...
        if inflow <= 0 or shed_tot + carried_tot - 99 + inflow <= 0:
            return
        need = shed_tot + carried_tot - 99 + inflow
        ...
        for item, q in buys.items():
            if need <= 0 or len(market) + len(added) >= MAX_ORDERS:
                break
