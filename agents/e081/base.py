...
def agent(observation,configuration=None):
    ...
    if pending:
        ...
        elif pending.get('count',2)==1:   # EXP350 sl: one-hand day, six-tile path from the donor's request
            state['workers'][pending['first']]=[tuple(t) for t in pending['targets']];_V233_REPORT['sheep_workers_confirmed']+=1;_SL_REPORT['confirmed']+=1
        else:
...
# ===== EXP342: layers from Thomas Tschinkel, "The 2945 Farm" (v9/4, public, Apache-2.0), appended on V48 =====
agent=_e335_agent
...
_CA_DROP = 0.0
_CA_BUFFER = 8
_CA_FEED_DAYS = 1   # v13: feed reserve 2 days -> 1
_CA_CASH = 800
_CA_RESCUE = True
...
import collections as _e342_collections
agent.telemetry=_e342_collections.ChainMap(_V9_COURIER_REPORT,_V9_CARROT_REPORT,_V9_HERD_REPORT,_V9_FERT_REPORT,_CA_REPORT,_OR2_REPORT,_CH_REPORT,_SR_REPORT,_HD2_REPORT,_CS_REPORT)
_e342_agent=agent

# EXP343 arm (carried over from EXP338) 'weedlag': weed-blocked BUILD_PASTURE/BUILD_COOP/PLANT recovery by one-step lag replay.
# The chassis already DIGs a weed under a PLANT/BUILD target and queues the intended action, but drops the queue as soon
# as the tape moves the unit away, so a pasture is never built and the cow bought for it stays in the shed all game
# (live: 2 of 126 games, about -1.5k..-2k each; V48 mirror on such a world loses ~2000 at that seat). After the DIG,
# replay the intended action next step, then keep that unit one step behind its tape for a bounded number of steps so
# the placement sequence (PICKUP -> walk -> PLACE) is executed intact. Pattern after salemali7, "Kaggriculture | 2900+"
# (public notebook, Apache-2.0), reimplemented on the V48 route tables. Other units and the market are untouched.
_E343_WL_REPORT=dict(events=0,replays=0,errors=0)
_E343_WL_STATE={}
_E343_WL_BASE=agent
_E343_WL_STEPS=8
_E343_WL_OPS=('BUILD_PASTURE','BUILD_COOP','PLANT')

def _e343_tape_units(player,t):
    native=_IMPL.chassis.players[player]
    if t<0 or t>718:return []
    e=_IMPL.chassis.routes[2 if t>=648 else native['route']][t]
...
def _e343_weedlag(obs,action):
    ...
    st['step']=step
    if step%24==0:st['active']={}   # hands are re-hired each day; lag state cannot survive the day boundary
    ...
    # 2. detect new weed-blocked build/plant events (the chassis has already turned them into DIG)
...
def _e343_weedlag_agent(observation,configuration=None):
    if int(observation.get('step',0))==0:
...
# EXP343 arm 'v233x' (original, on the tape's own V233 six-sheep project):
#  (1) day-11 commit: when two of the three shops known on day 11 are yarn stores, commit the expansion on day 11 (after the tape's
#      own SW land purchase and its native hires) instead of day 12, only if cash also covers every purchase and hire the tape still
#      plans through day 12. Sheep placed on day 11 produce on days 17, 20, 23, 26 and 29 (five harvests) instead of 18, 21, 24, 27.
#  (2) last days: no refresh after day 29 is sellable, so the expansion's hands skip CARE from day 28 (its bonus would pay after the
#      last sellable production), skip FEED on day 28 for sheep not producing tonight and skip FEED/PICKUP WHEAT/feed purchase on
#      day 29; on day 29 no hands are hired at all when no sheep holds wool.
# Everything else in V233 (workers, rescue feed, wool/fertilizer credits, sales) is unchanged; the functions are rebound by name.
_E343_YX_BASE=_e343_weedlag_agent
_E343_YX_REPORT=dict(yx_day11_checks=0,yx_day11_commits=0,yx_day11_declines=0,yx_care_skips=0,yx_feed_skips=0,yx_day29_norequest=0,yx_day29_nowheat=0,yx_errors=0)
_E343_YX_DAY11=True
_E343_YX_END=True
_E343_YX_TILES=[(x,5+i) for i in range(2) for x in range(5,8)]
_E343_YX_ORIG_REQUEST=_v233_request
_E343_YX_ORIG_WORKER=_v233_worker
_E343_YX_SEED={'WHEAT':10,'CARROT':20,'TOMATO':50,'STRAWBERRY':100,'MELON':80}
_E343_YX_ANIMAL={'SHEEP':500,'COW':400,'GOOSE':300}

def _e343_yx_tape_cost(obs,native,through_day):
    """Cash the tape itself still spends from the current step to the end of `through_day` (seeds, animals, products, land, hires)."""
    step=int(obs['step']);farm=obs['farms'][obs['player']];prices=obs['market']['prices'];total=0
    hires_today=int(farm['hires_today']);day0=step//24;route=native['route']
    for t in range(step,(through_day+1)*24):
        a=_IMPL.chassis.routes[2 if t>=648 else route][t];d=t//24
        if d!=day0 and t%24==0:hires_today=0
        for o in a.get('market',[]) or []:
            if not o:continue
            if o[0]=='BUY_SEED':total+=int(o[2])*_E343_YX_SEED.get(o[1],50)
            elif o[0]=='BUY_ANIMAL':total+=int(o[2])*_E343_YX_ANIMAL.get(o[1],500)
            elif o[0]=='BUY_PRODUCT':total+=int(o[2])*(int(prices.get(o[1],50))+10)
            elif o[0]=='BUY_LAND':total+=4000
            elif o[0]=='HIRE':total+=_v219_fib(hires_today);hires_today+=1
    return total

def _e343_yx_eligible11(obs,native):
    farm=obs['farms'][obs['player']];prices=obs['market']['prices']
    if len(farm['tiles'])!=10 or set(farm['unlocked_quadrants'])!={'NW','NE','SW'}:return False
    if obs['town']['unlocked_shops'][:3].count('YARN_STORE')<2 or prices['WOOL']<220 or prices['WHEAT']>45:return False
    if any(farm['tiles'][y][x]!='LOCKED' for y in (5,6) for x in range(5,8)):return False
    # a native sheep bought today and still in the shed is fine if the tape itself picks it up later today
    step=int(obs['step']);planned=_v219_native_day(native,11)
    pickups=sum(max(1,int(c[2]) if len(c)>2 else 1) for a in planned[step%24+1:] for c in [a.get('farmer') or ['PASS'],*(a.get('hands') or [])] if c and c[0]=='PICKUP' and len(c)>1 and c[1]=='SHEEP')
    if int(obs['private']['shed'].get('SHEEP',0))>pickups or any(i.get('SHEEP',0) for i in obs['private']['inventories']):return False
    for day in range(12,30):
        for a in _v219_native_day(native,day):
            if any(o and (o[0]=='BUY_LAND' or o[:2]==['BUY_ANIMAL','SHEEP']) for o in a.get('market',[])):return False
            if any(c and c[0] in ('PICKUP','PLACE') and len(c)>1 and c[1]=='SHEEP' for c in [a.get('farmer')]+a.get('hands',[])):return False
...
def _e343_yx_request(obs,action,state,native):
    step=int(obs['step']);day=step//24;hour=step%24;farm=obs['farms'][obs['player']]
    committed=state.get('committed')
    if _E343_YX_DAY11 and not committed and day==11:
        if state.get('requested_day')==day or hour<2 or hour>12:return action
        _E343_YX_REPORT['yx_day11_checks']+=1
        if not _e343_yx_eligible11(obs,native):return action
        planned=_v219_native_day(native,day)
        if any(o and o[0]=='HIRE' for a in planned[hour+1:] for o in a.get('market',[])):return action
        market=action.get('market',[]) or []
        if any(o and o[0]=='BUY_LAND' for o in market):return action
        parent_hires=sum(bool(o) and o[0]=='HIRE' for o in market)
        expected=max(len(a.get('hands',[])) for a in planned)
        if len(farm['hands'])+parent_hires!=expected:return action
        extra=[['BUY_LAND'],['BUY_ANIMAL','SHEEP',6],['BUY_PRODUCT','WHEAT',6],['HIRE'],['HIRE']]
        if len(market)+len(extra)>MAX_ORDERS:return action
        stock=projected_shed(action,FarmView(obs));incoming=12
        budget=7000+6*(int(obs['market']['prices']['WHEAT'])+10)+sum(_v219_fib(n) for n in range(farm['hires_today']+parent_hires,farm['hires_today']+parent_hires+2))
        for o in market:
            if not o:continue
            if o[0]=='BUY_PRODUCT':incoming+=int(o[2]);budget+=int(o[2])*(int(obs['market']['prices'][o[1]])+10)
            elif o[0]=='BUY_ANIMAL':incoming+=int(o[2]);budget+=int(o[2])*_E343_YX_ANIMAL[o[1]]
            elif o[0]=='BUY_SEED':budget+=int(o[2])*_E343_YX_SEED[o[1]]
        if sum(stock.values())+incoming>100:return action
        tape_cost=_e343_yx_tape_cost(obs,native,12)
        if farm['money']<budget+3000+tape_cost:
            _E343_YX_REPORT['yx_day11_declines']+=1;return action
        state['requested_day']=day;state['pending']={'first':expected+1,'initial':True}
        _V233_REPORT['sheep_hire_requests']+=2;_V233_REPORT['sheep_feed_buy_requests']+=6;_V233_REPORT['sheep_commit_requests']+=1
        _E343_YX_REPORT['yx_day11_commits']+=1;_E343_YX_REPORT['yx_commit_day']=11
        result=copy.deepcopy(action);result['market']=list(market)+extra
        return result
    if _E343_YX_END and committed and day==29 and state.get('requested_day')!=day:
        tiles=farm['tiles']
        if not any(isinstance(tiles[y][x],dict) and tiles[y][x].get('animal')=='SHEEP' and int(tiles[y][x].get('yield_units',0))>0 for x,y in _E343_YX_TILES):
            state['requested_day']=day;_E343_YX_REPORT['yx_day29_norequest']+=1;return action
        before=list(action.get('market',[]) or [])
        result=_E343_YX_ORIG_REQUEST(obs,action,state,native)
        if result is not action and state.get('requested_day')==day:
            m=list(result['market'])
            for i in range(len(m)-1,len(before)-1,-1):
                if m[i][:2]==['BUY_PRODUCT','WHEAT']:del m[i];_E343_YX_REPORT['yx_day29_nowheat']+=1;break
            result['market']=m
        return result
    return _E343_YX_ORIG_REQUEST(obs,action,state,native)

def _e343_yx_worker(obs,actor,targets):
    step=int(obs['step']);day=step//24
    if not _E343_YX_END or day<28:return _E343_YX_ORIG_WORKER(obs,actor,targets)
    farm=obs['farms'][obs['player']];private=obs['private']
    pos=tuple(farm['hands'][actor-1]);inv=private['inventories'][actor]
    access=((4,4),(5,4),(4,5),(5,5))
    home=min(access,key=lambda p:(abs(pos[0]-p[0])+abs(pos[1]-p[1]),p))
    distance=abs(pos[0]-home[0])+abs(pos[1]-home[1])
    cargo=[item for item in ('WOOL','FERTILIZER') if inv.get(item,0)]
    if cargo and step%24>=(22 if day==29 else 23)-distance:
        return _v219_walk(pos,home) or ['PLACE',cargo[0],inv[cargo[0]]]
    def producing_tonight(t):
        dsf=day+1-int(t.get('placed_day',0))-6;return dsf>=0 and dsf%3==0
    if day<29:
        hungry=sum(isinstance(farm['tiles'][y][x],dict) and farm['tiles'][y][x].get('animal')=='SHEEP' and not farm['tiles'][y][x].get('fed_today') and (producing_tonight(farm['tiles'][y][x]) or int(farm['tiles'][y][x].get('consecutive_unfed',0))>=1) for x,y in targets)
        if hungry and not inv.get('WHEAT',0) and private['shed'].get('WHEAT',0):
            return _v219_walk(pos,home) or ['PICKUP','WHEAT',min(hungry,private['shed']['WHEAT'])]
    tasks=[]
    for target in targets:
        ...
        if isinstance(tile,dict) and tile.get('animal')=='SHEEP':
            if day<29 and not tile['fed_today'] and (producing_tonight(tile) or int(tile.get('consecutive_unfed',0))>=1) and inv.get('WHEAT',0):command=['FEED']
            elif tile['yield_units']:command=['HARVEST']
            elif tile['fertilizer_available']:command=['COLLECT_FERTILIZER']
            elif not tile['fed_today'] and day<29 and not producing_tonight(tile):_E343_YX_REPORT['yx_feed_skips']+=1
            if not tile.get('cared_today') and command is None:_E343_YX_REPORT['yx_care_skips']+=1
        if command:tasks.append((abs(pos[0]-x)+abs(pos[1]-y),targets.index(target),target,command))
    if tasks:
        _,_,target,command=min(tasks);return _v219_walk(pos,target) or command
    if cargo:return _v219_walk(pos,home) or ['PLACE',cargo[0],inv[cargo[0]]]
...
_v233_request=_e343_yx_request
_v233_worker=_e343_yx_worker

def _e343_yx_day11(obs,configuration,action):
    """V233's per-step body for day 11 (the V233 agent itself returns before day 12): state, pending confirmation, request, workers, sales."""
    step=int(obs['step']);player=int(obs['player']);day=step//24
    state=_V233_STATES.get(player)
    # the base's V233 agent has already stamped this step into last_step (it returns before day 12); only a real rewind resets
    if state is None or step<state['last_step']:
        state={'last_step':step,'day':-1,'workers':{},'work':{},'credit':{'WOOL':0,'FERTILIZER':0}};_V233_STATES[player]=state
    state['last_step']=step
    if configuration is not None and any(configuration.get(k,v)!=v for k,v in (('boardSize',10),('turnsPerDay',24),('shedCapacity',100),('maxMarketOrdersPerTurn',10))):return action
    ...
    for actor,previous in state['work'].items():
        ...
        item={'HARVEST':'WOOL','COLLECT_FERTILIZER':'FERTILIZER'}.get(previous['command'][0])
        if item:
            gained=max(0,private['inventories'][actor].get(item,0)-previous['inventory'].get(item,0));state['credit'][item]+=gained
    pending=state.pop('pending',None)
    if pending:
        funded='SE' in farm['unlocked_quadrants'] and (not pending['initial'] or private['shed'].get('SHEEP',0)>=6)
        if not funded:_V233_REPORT['sheep_purchase_shortfalls']+=1
            ...
            _V233_REPORT['sheep_workers_confirmed']+=2
            if pending['initial']:state['committed']=True;_V233_REPORT['sheep_committed']+=1
    action=_e343_yx_request(obs,action,state,_IMPL.chassis.players[player])
    if not state.get('committed') or not state['workers']:return action
    result=copy.deepcopy(action)
    commands=[result.get('farmer') or ['PASS']]+list(result.get('hands') or [])
    commands+=[['PASS'] for _ in range(len(farm['hands'])+1-len(commands))]
    state['work']={}
    for actor,targets in state['workers'].items():
        if actor>=len(commands):continue
        command=_E343_YX_ORIG_WORKER(obs,actor,targets);commands[actor]=command
        state['work'][actor]={'step':step,'command':command,'inventory':dict(private['inventories'][actor])}
    result['farmer'],result['hands']=commands[0],commands[1:]
    result=_v234_rescue(obs,result,state)
    return result

def _e343_yx_agent(observation,configuration=None):
    action=_E343_YX_BASE(observation,configuration)
    try:
        if _E343_YX_DAY11 and int(observation['step'])//24==11:
            return _e343_yx_day11(observation,configuration,action)
    except Exception:
        _E343_YX_REPORT['yx_errors']+=1
    return action

_E343_LAST=_e343_yx_agent

def _e343_agent(observation,configuration=None):
    return _E343_LAST(observation,configuration)
import collections as _e343_collections
_e343_agent.telemetry=_e343_collections.ChainMap(_E343_WL_REPORT,_E343_YX_REPORT,agent.telemetry)

# ===== EXP350 (research194): one-worker non-harvest sheep service from Thomas Tschinkel, "The 2945 Farm" (v9/4, public, Apache-2.0) =====
from itertools import permutations as _r53_permutations
...
import collections as _e350sl_collections
_e343_agent.telemetry=_e350sl_collections.ChainMap(_SL_REPORT,_V233_REPORT,_e343_agent.telemetry)

# ===== EXP350: V219 tomato-crew skip from Thomas Tschinkel, "The Metav4 Farm" v13 (public, Apache-2.0) =====
# ---------------------------------------------------------------------------
...
import collections as _e350s_collections
_e343_agent.telemetry=_e350s_collections.ChainMap(_V13V_REPORT,_e343_agent.telemetry)

# ===== EXP350 entry: the packaged/loader entry point must be the last callable in the module =====
def _e350_agent(observation,configuration=None):
    return _e343_agent(observation,configuration)
_e350_agent.telemetry=_e343_agent.telemetry
