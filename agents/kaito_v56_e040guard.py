...
# ===== E040: フィードガード v2 (E038 の発火不足を修正) ==========================
# E039実戦 (102W42L) でも C5S4 が再発 (Shivi -53.9k / Fanch -40.3k)。
# 測定 (ep105828239): d10 に risk=0 のまま 8頭が未給餌で終わり、d11h0 risk=8。
# d11日中は PASS=0 (h6/h12/h18) で v1 (h12+・PASS限定) は h20-23 の3手しか
# 動けず救済失敗。教訓: ミラー -$48 を嫌って狭めたのは誤りだった。
# ミラーmarginはLBと無関係 (勝敗のみ・ミラーは実戦皆無) で、-$48/試合の
# コストに対し +$30k×6% = +$1.8k/試合の期待値。勝率基準では広い方が正しい。
# T1: 脱走前日個体 (unfed>=1 & 未給餌) がいれば終日 (h0-23) 発火。
# T2: h20+ に未給餌が6頭以上 (d10型の仕掛かり) なら発火。いずれもPASS限定。
_FEED_GUARD = True
_FEED_LATE_HOUR = 20
_FEED_LATE_UNFED = 6
...
def _feed_g(act, obs):
    ...
    try:
        ...
        if risk:
            targets = risk
        elif hour >= _FEED_LATE_HOUR and len(unfed) >= _FEED_LATE_UNFED:
            targets = unfed
        else:
            return act
        positions = [_sweep_get(farm, "farmer")] + list(_sweep_get(farm, "hands", []) or [])
