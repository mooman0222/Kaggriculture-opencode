"""E077: E072 + P2 d15ギャップフィル (暇な手が空タイル上にいて小麦種があれば PLANT WHEAT)。11チーム収斂の移植実験。"""
...
GAPFILL = os.environ.get("LA_GAPFILL", "1") == "1"
GAPFILL_DAY = int(os.environ.get("LA_GAPFILL_DAY", "12"))
...
class Live:
    def __init__(self, folder):
        self.b = _load(folder / "base.py", "e077_base")
    ...
    def _idle_work(self, act, farm, private=None, day=0):
        ...
        for i, (pos, a) in enumerate(zip(units, acts)):
            ...
            if tile is None:
                if (GAPFILL and i > 0 and day >= GAPFILL_DAY
                        and int((private.get("seeds") or {}).get("WHEAT", 0) or 0) > 0):
                    # P2: 暇な手の足元が空き・小麦種あり → その場で PLANT (移動なし)。
                    # 将来 PLANT 予定地との衝突リスクは単発・終盤限定で受容 (11チーム収斂)。
                    acts[i] = ["PLANT", "WHEAT"]
                continue
