"""E074: E072 + 朝レース用 MILK/WOOL 温存 (h20-23 の自前追加売りを抑制し h00 の弾にする。テープの売却・現金駆動は不変)。"""
...
SHED_CAP = 100
HOLD_AMMO = os.environ.get("LA_MILKHOLD", "1") == "1"  # h20-23 に MILK/WOOL の自前追加売りを抑えて朝レースの弾を温存
HOLD_ITEMS = ("MILK", "WOOL")



class Live:
    def __init__(self, folder):
        self.b = _load(folder / "base.py", "e074_base")
    ...
    def _dawn_guard(self, act, obs, step):
        ...
        if overflow <= 0:
            return
        items = sorted((i for i in PRODUCTS if shed.get(i, 0) > 0 and not (HOLD_AMMO and i in HOLD_ITEMS)),
    ...
    def _sell(self, act, obs, step):
        ...
        for item in items:
            ...
            if HOLD_AMMO and item in HOLD_ITEMS and (step % 24) >= 20:
                # 夜は朝レースの弾を温存: 現金駆動 (need) だけ残し、前倒し・棚圧リリーフは抑える
                q = min(avail, math.ceil(need / price)) if need > 0 else 0
            q = min(q, avail)
            if q > 0:
