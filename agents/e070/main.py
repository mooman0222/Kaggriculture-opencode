...
UMG_OPEN = int(os.environ.get("LA_UMG", "0"))  # LB 1 位 (Unknown Mother-Goose) の開幕小麦ミクロ構造を移植。0=off
UMG_MARKET = {0: [["BUY_PRODUCT", "WHEAT", 13], ["BUY_PRODUCT", "WHEAT", 5], ["SELL", "WHEAT", 13]],
              1: [["SELL", "WHEAT", 5], ["BUY_PRODUCT", "WHEAT", 5], ["HIRE"], ["HIRE"], ["HIRE"], ["HIRE"], ["HIRE"],
                  ["BUY_ANIMAL", "COW", 2], ["BUY_ANIMAL", "SHEEP", 2]]}
FEED_GUARD = int(os.environ.get("LA_FEED", "2"))  # 既定は 2 (暇な PASS のみ)。1/3 は A/B で負けた  # 0=off 1=PASS/CARE を差し替え 2=PASS のみ 3=空振り CARE (cared_today 済み) のみ  # 産出日に給餌が無いと貯めた CARE ボーナスが全部捨てられる (sim.hpp daily_refresh_animals)
ANIMAL_YIELD = {"GOOSE": (4, 1, 4), "COW": (8, 2, 6), "SHEEP": (6, 3, 6)}  # first_yield_day, interval, max_held
...
class Live:
    ...
    def act(self, obs):
        ...
        if UMG_OPEN and step in UMG_MARKET: act["market"] = [list(o) for o in UMG_MARKET[step]]
        ...
        try:
            ...
            if FEED_GUARD: self._feed_guard(act, farm, obs.get("private"), step // 24)
    ...
    def _feed_guard(self, act, farm, private=None, day=0):
        """産出日に給餌されていない動物の上に小麦持ちのユニットが立っていたら CARE/PASS を FEED に差し替える。
        エンジンは産出日に fed_today が偽だと貯めた pending_care_bonus を丸ごと捨てる。移動は一切変えない。"""
        ...
        for i, (pos, a) in enumerate(zip(units, acts)):
            op = a[0] if a else "PASS"
            if op not in ("PASS", "CARE"): continue
            if FEED_GUARD == 2 and op != "PASS": continue
            x, y = int(pos[0]), int(pos[1])
            tile = tiles[y][x]
            if not isinstance(tile, dict) or "animal" not in tile: continue
            if FEED_GUARD == 3 and op == "CARE" and not tile.get("cared_today"): continue
            if tile.get("fed_today") or (x, y) in fed_here: continue
            spec = ANIMAL_YIELD.get(tile.get("animal"))
            if not spec: continue
            first, interval, cap = spec
            since = (day + 1) - int(tile.get("placed_day", 0)) - first
            if since < 0 or since % interval: continue                     # 産出日ではない
            if int(tile.get("pending_care_bonus", 0)) <= 0: continue       # 捨てるボーナスが無い
            if int(tile.get("yield_units", 0)) >= cap: continue            # 上限で入らない
            if int((invs[i] if i < len(invs) else {}).get("WHEAT", 0) or 0) <= 0: continue
            acts[i] = ["FEED"]; fed_here.add((x, y))
