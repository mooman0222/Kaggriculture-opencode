...
# ===== E022b: セルフオラクル front-run ==========================================
# クローン戦の相手 = 自分と同一の v56。ファントムインスタンス (独立の状態を持つ
# もう1組のポリシー) に step+1 の観測を与えて「次ターンの売却予定」を予測し、
# プレミアム4品目を1ターン先回りして暴落前の価格を取る。
# v56 はリアクティブ (shed 残量から売却量を決める) ため、前倒し分はデット簿記
# なしで自動整合する (shed が減れば将来の売りが自然に縮む)。
_ORA_BB = _v51_build_policy(_V56_BACKBONE_ROUTE, _V54_CONFIG)
_ORA_YARN = _v51_build_policy(_V56_YARN_ROUTE, _V54_CONFIG)
_ORA_ITEMS = ("MELON", "MILK", "STRAWBERRY", "WOOL")
_ORA_SHOPS = {
    "BAKERY": ("EGG", "WHEAT"),
...
def _ora_phantom(obs, visible):
    step = int(_ora_get(obs, "step", 0) or 0) + 1
    pobs = dict(obs)
    pobs["step"] = step
    pobs["hour"] = step % 24
    pobs["day"] = step // 24
    if step < 72:
        d = _ORA_BB(pobs, visible)
        _ORA_YARN(pobs, visible)
        return d
    if _v56_first_shop(pobs) in _V56_CONTINUATION_SHOPS:
        return _ORA_YARN(pobs, visible)
    return _ORA_BB(pobs, visible)


def _ora_town_demand(obs, item, step):
    d = 1 if (item != "FERTILIZER" and step % 24 == 0) else 0
    if step % 4 != 0:
        return d
    for shp in list(_ora_get(_ora_get(obs, "town", {}) or {}, "unlocked_shops", []) or []):
        prods = _ORA_SHOPS.get(shp, ())
        if item in prods:
            d += 2 if len(prods) == 1 else 1
    return d


def _ora_front_run(action, obs, phantom_act):
    step = int(_ora_get(obs, "step", 0) or 0)
    private = _ora_get(obs, "private", {}) or {}
    shed = _ora_get(private, "shed", {}) or {}
    market = [list(o) for o in (action.get("market") or [])]
    units = [list(action.get("farmer") or ["PASS"])] +             [list(h or ["PASS"]) for h in (action.get("hands") or [])]
    pred = {}
    for o in (phantom_act.get("market") or []):
        if isinstance(o, (list, tuple)) and len(o) >= 3 and o[0] == "SELL" and o[1] in _ORA_ITEMS:
            pred[o[1]] = pred.get(o[1], 0) + max(0, int(o[2]))
    for item, tgt in pred.items():
        if tgt <= 0 or _ora_town_demand(obs, item, step) > 0:
            continue
...
_pre_oracle_agent = agent


def agent(obs, configuration=None):
    act = _pre_oracle_agent(obs, configuration)
    try:
        visible = _v56_visible_configuration(configuration)
        phantom = _ora_phantom(obs, visible)
        step = int(_ora_get(obs, "step", 0) or 0)
        if step >= 72 and step % 24 != 23:
            act = _ora_front_run(dict(act), obs, phantom)
    except Exception:
        pass
    return act
