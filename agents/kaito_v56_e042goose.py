...
_EAGER_ITEMS = ("STRAWBERRY", "MILK", "WOOL", "MELON", "EGG")
_EAGER_FROM_STEP = 72
...
# ===== E042: ヤーン店なしならガチョウ (2ca32a682f の農場分岐の移植) =======================
# 2ca32a682f は v56 と全手同一だが、d9 までの 4 店に YARN_STORE が無い場合のみ
# s218 SHEEP1→0 / s227 SHEEP1→GOOSE1 / s242 SHEEP2→GOOSE2 (C9S8→C9S5G3)。実戦 52 戦で
# 規則 28/28・24/24 一致。羊毛需要なし抽選 (羊毛 $33〜44) で羊 8 頭は死んだ市場に投げ、
# 卵 (~79 個 $4.3k) と羊 4 頭分 $2k の節約が入る。ボディはガチョウの配置・給餌を状態
# ベースで処理する (2ca の実戦で確認)。卵は eager 層 (_EAGER_ITEMS に EGG 追加) で売る。
_GOOSE_SWAP = True
_GOOSE_SWAP_STEPS = {217: 0, 226: "GOOSE", 241: "GOOSE"}  # obs.step は行動が記録される index の 1 つ前


def _goose_swap(act, obs):
    if not _GOOSE_SWAP:
        return act
    step = int(_sweep_get(obs, "step", 0) or 0)
    rule = _GOOSE_SWAP_STEPS.get(step)
    if rule is None:
        return act
    shops = _sweep_get(_sweep_get(obs, "town", {}) or {}, "unlocked_shops", []) or []
