"""Kaggriculture agent — E019b: HarvestForge-X (8C/4S コア) + 4ターン front-run.

出典: 公開ノートブック salemali7/kaggriculture-2900 (HarvestForge-X)。
LB 上位 (~2800-3000) はこのエージェントのクローン群で占められている
(上位8チームのリプレイ解析で牛8-9+羊4-5・イチゴ種44・雇用290人日の
同一シグネチャを確認、experiments.md E019 参照)。

構造: 上位3トレースの多数決で再構成した固定720手 (_ACTIONS) +
雑草修復 (意図復元つき) + プレミアム4品目の前倒し売り (front-run)。

E019b (クローン戦エッジ): front-run の先読みを1→4ターンに拡張し、品目別
デット方式で将来の SELL を相殺。クローンの投げ売りに先行して暴落前の
価格を取る。

検証 (2026-08-31, ローカル):
  - vs 素の HF: 19勝1敗0分 (10シード両席)、ペア margin +$5.1k (10/10 正)
  - vs base $153.2k (8シード、旧 E018 は $103.4k) / vs E018 5勝0敗
  - K4 同士のミラー: 両者 DONE・margin ±$1k (自壊なし)

提出形式: このファイルの `agent(obs)` をそのまま提出する。
旧 E018 実装は agents/e018_route.py (A/B 用に保持)。
"""
import base64
import copy
import json
...
# E019b: front-run の先読みを1→4ターンに拡張 (クローン戦エッジ)。
# LB 上位は同一クローンのため、プレミアム品の投げ売りに1-4ターン先行して
# 暴落前の価格を取る。素の HF に 19勝1敗・ペア margin +$5.1k (10/10 シードで正)。
# K=2 は +$4.6k、K=8 は +$3.9k で K=4 が最適 (先行しすぎは町需要の値上がりを逃す)
_FR_LOOKAHEAD = 4
_FR_STATE = {
    0: {"last_step": -1, "debt": {}},
    1: {"last_step": -1, "debt": {}},
...
def _future_quantity(step, item):
    tot = 0
    for s in range(step + 1, min(step + 1 + _FR_LOOKAHEAD, len(_ACTIONS))):
        for ord_ in (_ACTIONS[s].get("market") or []):
...
def _repay(action, state, step):
    debt = dict(state.get("debt") or {})
    if not any(v > 0 for v in debt.values()):
        return action
    act = _copy_action(action)
    new_mkt = []
    for r in act.get("market") or []:
        o = list(r)
        if len(o) >= 3 and o[0] == "SELL" and debt.get(o[1], 0) > 0:
            req = max(0, int(o[2]))
            red = min(req, debt[o[1]])
            req -= red
            debt[o[1]] -= red
            if req <= 0:
                continue
...
def _front_run(action, obs, state, step):
    ...
    for itm in _FR_ITEMS:
        tgt = _future_quantity(step, itm) - max(0, int((state.get("debt") or {}).get(itm, 0)))
    ...
    if mvd:
        ...
        for itm, q in mvd.items():
            debt[itm] = debt.get(itm, 0) + q
    return act
