...
## 6. 関門パネル (2026-09-20、E076 不発の反省から)

身内同士・古い窓だけの評価では 50 位判定はできない (E076: 素体 +1,458 が実戦で停滞)。
**提出前には関門パネル (`.opencode/data/gate_panel.json`) の 5 世界で両スロットを席差し替えする**:
現行パネルは両スロットが共に負ける 2500+ 帯 (jojo −4573/−2806、Dude −1069/−380、
IamDiganta −715/−78、CrazyML −26/−2419、Xiaowenhao −319/−757)。
判定は**勝数**で行う (LB は勝敗のみ。+460 点のマージンは勝数+1 にしか化けなかった)。
リプレイは `tmp/e072_battles/` + `tmp/e076_battles/` (git 管理外。消えたら episode ID で再取得)。
パネルは実戦の負け筋が変わったら更新する (2500+ 帯の新規負けを追加、古い版の世界は落とす)。
