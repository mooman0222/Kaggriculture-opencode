...
- ~~2900 超はライブ方策のみ~~ **訂正 (09-18)**: 現 1 位は開幕 6 日が固定テープの混成。`refs/top-survey-2026-09-18.md`
...
## E055 (2026-09-11): 0909 土台への移行と開幕ガード (ref 56156691)
- E054 は 2740 をピークに 2590 へ後退。直近60戦の相手は 50 が **Shop Router 0909** (09-09 公開、13ルート・1ターン先回し・雑草 DIG、h72=b3105bcc = 旧「SR0908v」= mtmr_s1 系) の複製・派生で 20勝30敗。
  0908 土台の E054 は 0909 と五分 (直接対決 15勝9敗 −37)。0908 無改変コピー (56111494) は 2049 に沈んだ
- **開幕荒らし系 a17674d6 (5チーム) に 0勝5敗 −73k**: t0 に小麦 13+60 買い→60 売りで我々の t0 往復 (買13/売13/買13) を −400 にし、t1 の羊2頭 (1000) を資金不足で失敗させる。羊なしのルートは −70k
- 上位15: ライブ方策が過半 (SpaTaro 3142、ymg_aq、Majkel1337、binghua、c0nrad、keiz、Otter Vibe)、Thomas 開幕 + ライブ後半 (Mengfei Li、feel the agi)、0909 系は3 (Himanshu 2955、Gleb 2968、mtmr_s1 2892)。50位 = 2854、25位 = 2894
- 実装 `agents/sr0909_live/` (router.py = 0909 原本): クローンゲート、K=6 先回し (0909 自前の1ターン先回しをクローン時のみ置換、同ターン DROP 投影 `projected_shed` を流用)、
  暇な手 WATER/CARE、**開幕ガード** (t0 の小麦往復を撤去、t1 は家畜購入を雇用より先に)。非クローン相手には開幕ガード以外 0909 と同一
- 計測: 0909 ミラー 24勝0敗 +2.9k。E054 実戦60戦の自席差し替え (`tests/eval_replays.py --team MMN0222 --base agents/sr0909_base/main.py`):
  素の 0909 2勝 → 本版 53勝。0909 系 47/50 (+11.8k)、荒らし 5/6 (−73k → ±2k)、他は差分 ≤10
- 新ツール: `tests/fetch_sub.py` (提出IDのリプレイ取得)、`tests/lineage_census.py` (自リプレイの相手系統別集計)、`tests/xray.py` (チーム別系統・プラン一致率)、`tests/quick.py`
