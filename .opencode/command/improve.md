...
1. **仮説選定**: `.opencode/knowledge/tracks/*.md` の「生きている問い」と `AGENTS.md`「次にやること」から 1 つ選ぶ。無ければ該当 track の 6 節に新規追加
...
5. **反映**: 有意な改善なら `.opencode/knowledge/refs/engine_facts.md` を更新 (日付・条件付き) し、`agents/kaito_v56_orak16.py` のデフォルトに反映。既存知見と矛盾したら実験結果を優先して上書きする
