...
- bc19 (初期値 bc9k を公開データ 634→2,132 局で焼き直し → Majkel fine-tune) は Kaggle `kaggriculture-bc19-policy3` (`rl/kaggle15/`) で実行中。
- **bc20 = Majkel 854 局のみ・初期値なし 4 epoch → 32 戦 92.1k (bc18 92.8k と SE 内)**。混合初期値 bc9k の寄与は測定限界以下。bc20 は 4 epoch で未収束 (ep2 62.3k → ep3 92.1k) なので継続 (bc21) 中。
