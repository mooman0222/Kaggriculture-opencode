...
1. **クローン戦のエッジ開発 (最優先)**: HF mirror は margin ≤$1k か完全同点。相手の
   _ACTIONS (完全な売買計画) を我々は把握しているので、市場側のみの微修正
   (売りタイミングの前倒し・数量調整) で一貫した +$1-2k を作れればクローン群を
   ほぼ総なめにできる = レーティングはクローン天井 (~2900) の上へ。農場側は触らない
   (E018-M7 の教訓はこのコアにも適用: 固定手はタイル占有介入を受け付けない前提で扱う)
2. **提出判断**: 新 main.py は現行 LB 提出 (753.8) を大差で上回る見込み。再提出は
   600 からのやり直しだが、出力 $153.6k 級ならクローン帯まで数時間で収束するはず
   (勝率 ~80%+ 前提)。提出はユーザー確認の上で
3. **小麦アービトラージ (未検証)**: タウン需要で小麦価格は 25→40-47 に単調上昇。
   早期買い・後期売りの構造的エッジ候補 (市場側のみ)。クローン戦エッジの素材にもなる
