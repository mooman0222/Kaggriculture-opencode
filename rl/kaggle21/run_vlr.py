"""Kaggle GPU: value head を専用 lr で学習させると PPO 崩壊が止まるか。

診断 (2026-09-19, `rl/diag_adv.py`) の結論:
  崩壊後の ckpt はどれも V がほぼ定数 (MC リターン var ≈ 1.03 に対し R² 0.00〜0.01、v_std 0.08、
  最終手の符号一致 0.56)。同じ凍結特徴で value head を lr 1e-3 で焼き直すと R²_val 0.44〜0.58。
  つまり critic は「表現できない」のではなく「学習していない」 — bc5_ep3 の value head は
  アーキテクチャ不一致で乱数初期化なのに、BC を守る lr 1e-5 (200 step ウォームアップ) と
  60 step の暖機しか与えられていない。その結果 adv ≈ 生の outcome になり、pg が高分散ノイズになる。

この腕は SP3 と同じレシピで value head だけ速く動かす (`--vlr 1e-3 --vwarmup 20`、他は不変)。
  崩壊が止まる/緩む -> critic が主因 (次の段階へ)
  変わらない      -> RL を凍結して本線 (chassis) へ

判定: 毎 iter の vsT / vsTAPE window と、対 v41 32 戦 own (基準 63.1k、同レシピ real/base は final 17.4k)。

乾式実行: KAGGLE_ROOT=<偽ツリー> KAGGLE_DRY=1 .venv/bin/python rl/kaggle21/run_vlr.py
結果取得: .venv/bin/python -m kaggle kernels output mmn0222/kaggriculture-ppo-vlr -p tmp/kaggle_out_vlr --force
"""
import glob
import os
import shutil
