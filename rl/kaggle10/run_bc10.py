"""Continue Policy3 (bc9k_ep3) for 4 more epochs; same 1,200-game split, lower LR."""
import glob
import os
import shutil
import subprocess
...
command = [
    ...
    "--out", os.path.join(output, "bc10k.pt"),
    "--init", initial,
    "--epochs", "4",
    "--bs", "128",
    "--lr", "2e-4",
...
if have_simulator and result.returncode == 0:
    ...
    with open(os.path.join(output, "eval.txt"), "w") as report:
        for checkpoint in sorted(glob.glob(os.path.join(output, "bc10k_ep*.pt"))):
