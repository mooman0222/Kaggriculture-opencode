...
| bc21 | bc20_ep3 から Majkel 854 局で 2 epoch 継続 lr 2e-4 | 実行中 (09-17) | 上り坂の途中で止めた bc20 の続き |
...
バケツに 5,7,9,10 を足せば完全に直るが `N_MB` が変わり market ヘッドの重みを引き継げない。**まず 1 行版を bc21 と同じレシピで A/B する**。
...
1. **市場バケツの同点丸めを直す** (上の監査、1 行): `bucket()` の同点を大きい方へ。bc21 と同じレシピで A/B。売り +28 個/局・買い小麦 +6 個/局 相当。
2. **収穫と給水のスループット** (本命、未着手): bc18 は HARVEST 411 / WATER 1259 に対し Majkel は 497 / 1387。得点差はここ。
   メロンの収穫齢は 09-16 に診断済み (滞留 0.22 日、バグなし) だが **他の作物は未調査**。まず作物別に「実った株が収穫されるまでの滞留」を測る。
3. **bc19 の結果を見る** (Kaggle 実行中、下記): 初期値 bc9k を公開データ 2,132 局 (旧 634) で焼き直し → Majkel 854 局で fine-tune。データ量の梃子が初期値側にも効くかの検証。
4. ~~Majkel 単独データのみ (初期値なし) の学習~~ **済 (bc20)**: 32 戦 92.1k/−21.4k で bc18 (92.8k) と SE 内 → **混合初期値 bc9k の寄与は測定限界以下**。
   ただし bc20 の epoch 曲線は ep2 62.3k → ep3 92.1k とまだ上り坂の途中。継続 (bc21) を実行中。
5. データ増量の継続: Majkel は 09-17 に +141 局 (854)。`fetch_episodes.py` で日次追加。
6. やらないこと: **fine-tune の** epoch 追加 (bc16 で崩壊、混合初期値 4 + fine-tune 4〜5 が上限。ただし**スクラッチ学習にはこの上限は当てはまらない** — bc20 は 4 epoch で未収束)、PPO (再凍結、規模が前提)、農場側の推論強制・班分け・待機ラベル・自己軌跡・温度サンプリング・容量 d256、
