"""Kaggle GPU: PPO 崩壊の機構を 4b (昇格停止) と 4c (entropy 賞与 0) の実走で切る。

シャッフル対照 (2026-09-18) の結論: pg を無効化した shuf は同じ 45 iter でも劣化が半分以下の速さ
(vsT window 傾き −0.033 対 real −0.081/10it、最終 own 40.4k)。信号に従うことが崩壊を悪化させる。
残差の drift (基準 63k → 40k) は entropy 上昇 (0.30→0.45) と昇格済み錨が候補。

  nopromo : --promote 2.0 (勝率上限 1.0 超 = 昇格を一度も起こさない。KL 錨は BC のまま)
  ent0    : --ent 0     (entropy 賞与を切る)

他は SP3 と同一 (Policy2、bc5_ep3 初期値、tape-frac 0.25、T 96、45 iter、150 分上限)。
判定: 毎 iter の vsT / vsTAPE window の水準と傾き (real/shuf の既走ログと比較) と
対 v41 32 戦 own (基準 63.1k)。nopromo が崩れなければ 4b が主因、ent0 が shuf の残差 drift を
消せば 4c。

乾式実行: KAGGLE_ROOT=<偽ツリー> KAGGLE_DRY=1 .venv/bin/python rl/kaggle20/run_4b4c.py
結果取得: .venv/bin/python -m kaggle kernels output mmn0222/kaggriculture-ppo-nopromo-ent0 -p tmp/kaggle_out_4b4c --force
"""
import glob
import os
import shutil
...
ARMS = [("nopromo", ["--promote", "2.0"]), ("ent0", ["--ent", "0"])]
...
def patched_rl_dir():
    """--shuffle-adv / --greedy-dest を持つ train.py がある rl/ を選ぶ (bc7 dataset に古い複製が同居しているため)。"""
...
for name, extra in ARMS:
    ...
    started = time.time()
    result = subprocess.run([sys.executable, os.path.join(rl, "sp", "train.py"), "--out", out, *extra, *common],
...
with open(os.path.join(output, "eval.txt"), "a") as report:
    targets = []
    for name, _ in ARMS:
        targets += sorted(glob.glob(os.path.join(output, f"ctl_{name}_it*.pt"))) + [os.path.join(output, f"ctl_{name}.pt")]
