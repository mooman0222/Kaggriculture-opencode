"""Continue Policy3 (bc9k_ep3) for 4 epochs with PASS_IDLE_WEIGHT=0.1 (idle labels while work remains down-weighted); code from the rl-code dataset."""
import glob
import os
import shutil
import subprocess
...
command = [
    ...
    "--out", os.path.join(output, "bc11k.pt"),
    "--init", initial,
    "--epochs", "4",
    "--bs", "128",
    "--lr", "2e-4",
    "--pass-idle-weight", "0.1",
    "--max-games", "1200",
]
started = time.time()
...
if have_simulator and result.returncode == 0:
    ...
    with open(os.path.join(output, "eval.txt"), "w") as report:
        for checkpoint in sorted(glob.glob(os.path.join(output, "bc11k_ep*.pt"))):
