"""Kaggle GPU run: behaviour cloning bc6 (winner-only shards) + closed-loop eval vs v41 in kagsim. Outputs in /kaggle/working."""
...
code = os.path.join(root, "code"); rl = os.path.join(code, "rl"); data = os.path.join(root, "data", "bc6data"); out = "/kaggle/working"
...
# resume support: a previous run's output attached as a dataset provides bc6k.pt.state
prev = glob.glob("/kaggle/input/**/bc6k.pt.state", recursive=True)
if prev: shutil.copy(prev[0], os.path.join(out, "bc6k.pt.state")); print("resuming from", prev[0], flush=True)
cmd = [sys.executable, os.path.join(rl, "train_bc2.py"), "--data", os.path.join(data, "*.npz"), "--out", os.path.join(out, "bc6k.pt"), "--epochs", "6", "--bs", "1024", "--lr", "5e-4"] + (["--resume"] if prev else [])
...
if have_sim:
    ...
    with open(os.path.join(out, "eval.txt"), "a") as f:
        for ck in sorted(glob.glob(os.path.join(out, "bc6k_ep*.pt"))):
