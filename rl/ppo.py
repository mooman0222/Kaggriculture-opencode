"""PPO fine-tuning from a BC checkpoint against an opponent pool. 使い方:
...
from rollout import rollout, logp_of, ScriptedOpp
...
def main():
    ...
    opps = [ScriptedOpp(p) for p in a.opps]; opening = ScriptedOpp("agents/e058/main.py") if a.opening else None
    for it in range(a.iters):
        ...
        for ep in range(a.epochs):
            ...
            for b in range(0, N, a.bs):
                ...
                out = model(bt["tiles"], bt["units"], bt["items"], bt["glob"])
                logp, ent = logp_of(out, bt["mask"], {k: bt[k].long() for k in ("op", "qty", "sell", "buyp", "seed", "anim", "hire", "land")})
                ratio = torch.exp(logp - bt["logp"]); A = adv_t[idx].to(dev)
                pg = -torch.min(ratio * A, torch.clamp(ratio, 1 - a.clip, 1 + a.clip) * A).mean()
                vl = ((out["value"] - ret_t[idx].to(dev)) ** 2).mean(); loss = pg + a.vf * vl - a.ent * ent.mean()
